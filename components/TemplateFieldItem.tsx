...
export default function TemplateFieldItem({ field, onEdit }: Props) {
    ...
    return (
        <div 
            ref={setNodeRef} 
            style={style} 
            className={`group flex gap-4 p-2 border border-(--primary)/30 bg-black/40 hover:border-(--primary)/70 transition-all ${isDragging ? 'border-(--primary) shadow-lg' : ''}`}
        >
            {/* Drag Handle */}
            <div 
                {...attributes} 
            ...
            <div className="flex-1 min-w-0">
                <p className="text-[9px] text-(--primary)/60 font-mono tracking-tighter">
                    {"{{" + field.variable_name + "}}"}
                </p>
                <h4 className="font-Google-Sans font-bold truncate uppercase text-sm leading-tight">
                    {field.label}
                </h4>
