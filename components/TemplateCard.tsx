...
export default function TemplateCard({ item, viewMode, isAdmin, onTagClick, onDelete, onOpenPrivateModal }: any) {
    ...
    if (viewMode === 'line') {
        return (
            <div className="zzzcode-list-item border border-(--primary) p-2 bg-(--background) transition-all group relative items-center">
                ...
                <div className="hidden md:block aspect-square w-full overflow-hidden border border-(--primary)/20 relative after:content-[''] after:absolute after:inset-0 after:mix-blend-color after:bg-(--primary) after:opacity-100 after:select-none after:pointer-events-none group-hover:after:opacity-0 group-active:after:opacity-0 after:transition-all after:duration-500">
                    <img 
                        src={item.preview_url || '/placeholder.png'} 
                        className="w-full h-full object-cover transition-all duration-500 saturate-0 group-hover:filter-none group-hover:scale-105 group-active:filter-none group-active:scale-105"
                        alt={item.title}
                    />
                </div>
                    ...
                    <div className="hidden md:flex flex-wrap gap-1 items-stretch">
                        {item.template_tags.map((t: any) => (
                            <button 
                                ...
                                className="border border-(--foreground) bg-(--foreground) text-(--background) px-1.5 py-0.5 text-xs font-bold lowercase hover:bg-(--background) hover:text-(--foreground) active:bg-(--background) active:text-(--foreground) transition-all duration-300 ease-in-out cursor-pointer whitespace-nowrap"
                            >
                                {t.tags.name}
                            </button>
                        ))}
                ...
                <div className="flex flex-wrap gap-1 items-center">
                    ...
                    <button 
                        ...
                        title="Use Template"
                        className="p-1.5 border border-(--primary)/30 text-(--primary) hover:bg-(--primary) hover:text-black active:bg-(--primary) active:text-black transition-colors duration-300 ease cursor-pointer"
                    >
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    ...
                    {isAdmin && (
                        <>
                            ...
                            <button
                                ...
                                className="p-1.5 border border-red-500/30 text-red-500 hover:bg-red-500 hover:text-white active:bg-red-500 active:text-white transition-colors duration-300 ease cursor-pointer"
                            >
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    ...
    return (
        <div className="zzzcode-card-item border border-(--primary) p-2 bg-(--background) transition-all group relative">
            {/* Preview Image + Orange Filter */}
            <div className="aspect-square w-full overflow-hidden border border-(--primary)/20 relative after:content-[''] after:absolute after:inset-0 after:mix-blend-overlay after:bg-(--primary) after:opacity-100 after:select-none after:pointer-events-none group-hover:after:opacity-0 group-active:after:opacity-0 after:transition-all after:duration-500">
                <img 
                    src={item.preview_url || '/placeholder.png'} 
                    className="w-full h-full object-cover transition-all duration-500 saturate-0 group-hover:filter-none group-hover:scale-105 group-active:filter-none group-active:scale-105"
                    alt={item.title}
            ...
            <div className="flex flex-wrap gap-1 items-stretch">
                ...
                {item.template_tags?.length > 2 && (
                    <div className="group/tooltip flex relative">
                        <button className="border border-(--foreground) bg-(--background) text-(--foreground) px-1.5 py-0.5 text-xs font-bold hover:bg-(--foreground) hover:text-(--background) active:bg-(--foreground) active:text-(--background) transition-all cursor-help">
                        ...
                        <div className="invisible group-hover/tooltip:visible group-active/tooltip:visible opacity-0 group-hover/tooltip:opacity-100 group-active/tooltip:opacity-100 absolute bottom-full left-0 mb-2 p-2 bg-(--background) border border-(--primary) z-50 min-w-[120px] transition-all duration-200">
