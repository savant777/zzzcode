...
export default function TemplateGroupContainer({ id, groupName, gIdx, groupFields, onFieldDragEnd, sensors, onEdit }: any) {
    ...
    return (
        <div ref={setNodeRef} style={style} className="flex flex-col gap-2 mb-4">
            {/* Group Header (Drag Handle) */}
            <div 
                {...attributes} 
                {...listeners} 
                className={`
                    flex items-center gap-2 px-2 py-1 bg-(--primary)/5 border-l-2 border-(--primary) cursor-grab active:cursor-grabbing hover:bg-(--primary)/10 transition-colors
