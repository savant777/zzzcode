...
async function main() {
    ...
    let unlock = load();
    assert.equal(await unlock.isTemplateUnlocked('1', 'secret'), false);
    assert.equal(await unlock.rememberTemplateUnlock('1', 'secret'), true);
    assert.equal(await unlock.isTemplateUnlocked('1', 'secret'), true);
    // Simulate closing and reopening the browser: keep local storage only.
    window.sessionStorage = storage();
    unlock = load();
    assert.equal(await unlock.isTemplateUnlocked('1', 'secret'), true);
    assert.equal(await unlock.isTemplateUnlocked('2', 'secret'), false);
    assert.equal(await unlock.isTemplateUnlocked('1', 'changed'), false);
    assert.equal(await unlock.isTemplateUnlocked('1', null), false);
    assert.equal(await unlock.rememberTemplateUnlock('1', 'changed'), true);
    unlock = load();
    assert.equal(await unlock.isTemplateUnlocked('1', 'changed'), true);
    assert.equal(await unlock.isTemplateUnlocked('1', 'secret'), false);
    ...
    assert.equal(await unlock.isTemplateUnlocked('3', 'secret'), false);
    ...
    unlock = load();
    assert.equal(await unlock.isTemplateUnlocked('4', 'secret'), true);
    ...
    assert.equal(await unlock.isTemplateUnlocked('5', 'secret'), true);
    assert.equal(await unlock.isTemplateUnlocked('5', 'changed'), false);
    assert.equal(await unlock.isTemplateUnlocked('6', 'secret'), false);
